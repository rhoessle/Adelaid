...
file_in  = '/home/ralf/Schreibtisch/Programming_Language/Python/Python_Learning/Python_Files/converters/Fiorano/SDS/Fiorano_SDS.xlsm'
file_out = '/home/ralf/Schreibtisch/Programming_Language/Python/Python_Learning/Python_Files/converters/Fiorano/SDS/TEST_1.json'

# Load Excel workbook
wb = load_workbook(file_in, data_only=True)
#wb = load_workbook('/home/rhoessle/rhoessle_fiorano_viborg.Wtmp/fiorano/www/private/specs/Apple/Top/Fiorano_SDS.xlsm',               data_only=True)
...
for x in range(column_GPIO_start, column_GPIO_end+1, 3):
  ...
  for y in range(row_min_specpar, row_max_specpar+1):
    ...
    value_unit              = str(sheet.cell(row=y,     column=column_unit             ).value)	;# VDD0
    ...
    if value_primary_parameter == 'None' or value_primary_parameter == None:
      value_primary_parameter = '-'
    if bool(re.match('.*setting ',value_primary_parameter)):
      setting = re.sub('.*setting ', '', value_primary_parameter)
    else:
      setting = '-'
    value_min  =     sheet.cell(row=y,     column=x-1           ).value 	;# 'None'
    if value_min == 'None' or value_min == None:
      value_min = '-'
    value_typ =      sheet.cell(row=y,     column=x             ).value		;# 'None'
    if value_typ == 'None' or value_typ == None:
      value_typ = '-'
    value_max =      sheet.cell(row=y,     column=x+1           ).value 	;# 'None'
  ...
  for xf in range(0, 19+1, 1):
    ...
    if value_feature == 'None' or value_feature == None:
      value_feature = '-'
    #dict_features['features'][key_feature] = value_feature
